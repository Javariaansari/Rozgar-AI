...
export async function translateToEnglish(text) {
  ...
  if (useMock) {
    console.log('[MOCK AI] translateToEnglish')
    return text.trim()
  }

  try {
    const model = genAI.getGenerativeModel({ model: 'gemini-3.5-flash' })
    const prompt = `Translate the following text into clear, natural English. If the text is already in English, just return it as-is. Keep the same meaning and tone. Return ONLY the translated text, with no extra explanation, no markdown, and no quotes around the output.
